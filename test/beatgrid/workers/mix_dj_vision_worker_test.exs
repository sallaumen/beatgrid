defmodule Beatgrid.Workers.MixDjVisionWorkerTest do
  ...
  test "samples grids, OCRs, and writes :image dj parts" do
    mix = insert(:mix, duration_ms: 40_000, source_url: "https://youtu.be/x")
    insert(:mix_segment, mix: mix, position: 0, start_ms: 0)
    insert(:mix_segment, mix: mix, position: 1, start_ms: 20_000)

    expect(Beatgrid.Video.FrameSamplerMock, :resolve_stream, fn _ -> {:ok, "http://stream"} end)
    stub(Beatgrid.Video.FrameSamplerMock, :sample_grid, fn _u, %{dest: dest} -> {:ok, dest} end)

    stub(Beatgrid.AI.Mock, :complete, fn _p, _s, _o ->
      {:ok, %{"tiles" => [%{"ts_ms" => 0, "dj_name" => "A"}, %{"ts_ms" => 20_000, "dj_name" => "B"}]}}
    end)
