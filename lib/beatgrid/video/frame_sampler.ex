defmodule Beatgrid.Video.FrameSampler do
  ...
  @callback download_video(url :: String.t(), dir :: String.t()) ::
  ...
  @callback extract_frames(
              video_path :: String.t(),
              opts :: %{interval_ms: integer(), dir: String.t()}
            ) ::
              {:ok, [String.t()]} | {:error, term()}

  @doc "Tile already-cropped local frames into one montage image (reading order)."
  @callback montage(frame_paths :: [String.t()], dest :: String.t()) ::
              {:ok, String.t()} | {:error, term()}
end
