defmodule BeatgridWeb.Router do
  use BeatgridWeb, :router

  pipeline :browser do
    plug :accepts, ["html"]
    plug :fetch_session
    plug :fetch_live_flash
    plug :put_root_layout, html: {BeatgridWeb.Layouts, :root}
    plug :protect_from_forgery
    plug :put_secure_browser_headers
  end

  pipeline :api do
    plug :accepts, ["json"]
  end

  # Quietly absorb the zombie service-worker poll some browsers cache for this
  # localhost port (we ship no PWA). No pipeline needed — it just serves JS.
  scope "/", BeatgridWeb do
    get "/sw.js", ServiceWorkerController, :unregister
  end

  scope "/", BeatgridWeb do
    pipe_through :browser

    live_session :default do
      live "/", LibraryLive, :index
      live "/track/:id", TrackLive, :show
      live "/revisao", ReviewLive, :index
      live "/dedup", DedupLive, :index
      live "/resgate", RescueLive, :index
      live "/painel", DashboardLive, :index
      live "/set", RecSetLive, :index
      live "/set/:id", RecSetLive, :show
      live "/discotecagem", DiscotecagemLive, :index
      live "/jobs", JobsLive, :index
      live "/ajustes", AjustesLive, :index
      live "/generos", GenresLive, :index
      live "/importados", ImportsLive, :index
      live "/sets-online", MixesLive, :index
      live "/sets-online/:id", MixLive, :show
    end

    get "/audio/:id/original", AudioController, :original
    get "/audio/:id", AudioController, :show
    get "/sets-online/:id/audio", AudioController, :mix
  end

  # Other scopes may use custom stacks.
  # scope "/api", BeatgridWeb do
  #   pipe_through :api
  # end

  # Enable LiveDashboard in development
  if Application.compile_env(:beatgrid, :dev_routes) do
    # If you want to use the LiveDashboard in production, you should put
    # it behind authentication and allow only admins to access it.
    # If your application does not have an admins-only section yet,
    # you can use Plug.BasicAuth to set up some basic authentication
    # as long as you are also using SSL (which you should anyway).
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through :browser

      live_dashboard "/dashboard", metrics: BeatgridWeb.Telemetry
    end
  end
end
