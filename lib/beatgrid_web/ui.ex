defmodule BeatgridWeb.UI do
  @moduledoc """
  Beatgrid design-system building blocks: the small recurring function
  components (badges, chips, Camelot seal, cover) plus delegates to the pure
  display formatters in `BeatgridWeb.UI.Format`.
  """
  use Phoenix.Component

  alias Beatgrid.Library.Marker
  alias BeatgridWeb.UI.Format
  alias Phoenix.LiveView.JS

  defdelegate folder_color(key), to: Format
  defdelegate folder_label(key), to: Format
  defdelegate cover_src(track), to: Format
  defdelegate rating_color(rating), to: Format
  defdelegate camelot_color(code), to: Format
  defdelegate confidence_color(level), to: Format
  defdelegate confidence_label(level), to: Format
  defdelegate format_lufs(lufs), to: Format
  defdelegate format_gain(gain), to: Format
  defdelegate loudness_delta_class(delta), to: Format
  defdelegate format_views(views), to: Format
  defdelegate format_age(date), to: Format
  defdelegate format_ms(ms), to: Format
  defdelegate effective_bpm(track), to: Format
  defdelegate effective_camelot(track), to: Format
  defdelegate initials(artist), to: Format
  defdelegate cover_gradient(seed), to: Format
